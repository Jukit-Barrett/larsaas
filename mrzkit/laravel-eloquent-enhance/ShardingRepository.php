...
use Mrzkit\LaravelEloquentEnhance\Exceptions\ShardingException;

abstract class ShardingRepository implements ModelContract, RepositoryContract, ResolverContract, TrashContract
{
    use ResolverTrait;
    ...
    public function create(array $data)
    {
        ...
        if (!$model->fill($data)->save()) {
            throw new ShardingException("Create Model fail.");
        }
    ...
    public function update(int $id, array $attributes): Model
    {
        ...
        if (is_null($object)) {
            throw new ShardingException("Update failure, Resource not found.");
        }

        if (!$object->update($attributes)) {
            throw new ShardingException("Update failure.");
        }
    ...
    public function delete(int $id): bool
    {
        ...
        if (is_null($object)) {
            throw new ShardingException("Delete failure, Resource not found.");
        }

        $deleted = $object->delete();

        if (!$deleted) {
            throw new ShardingException("Delete failure.");
        }
